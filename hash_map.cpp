...
public:
    void insertNode(int data) {
        ...
        frequencyMap[data]++;
        // Insert element into the list only if it's not already there
...
int main() {
    ...
    for (int i = 0; i < numberOfNodes; i++) {
        int data = rand() % 100 + 1;
        hashMap.insertNode(data);
    }

    int totalElements = 40;
    int uniqueNodes = hashMap.getNumberOfNodes();
    if (uniqueNodes == 0) {
        cout << "There are no numbers in the list";
    } else {
    ...
    hashMap.getFrequentElement();
    cout << "The smallest number is " << hashMap.getSmallestElement() << " and its count is " << hashMap.getCountOfSmallestElement() << ".\n";
    cout << "The largest number is " << hashMap.getLargestElement() << " and its count is " << hashMap.getCountOfLargestElement() << ".\n";
    cout << "---------------------------------------------------------" << endl;
    ...
    while (true) {
        ...
        hashMap.searchForNode(searchNumber);
        cout << "---------------------------------------------------------" << endl;
    }
